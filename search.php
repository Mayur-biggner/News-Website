    ...
    <div class="container content">
        <div class="row mt-5">
            <div class="col col-7 bg-white ms-5 m-3">
                ...
                $offset = ($page - 1) * $limit;
                //  this below query is used for fetching all record for the serach term.
                $sql = "SELECT post.post_id,post.category,post.description,post.author,post.post_img,post.title,category.category_name,user.username,post.post_date,category.category_id FROM post LEFT JOIN category ON post.category=category.category_id LEFT JOIN user ON post.author=user.user_id WHERE post.author LIKE '%{$search_term}%' OR post.title LIKE '%{$search_term}%' OR user.username LIKE '%{$search_term}%' OR category.category_name LIKE '%{$search_term}%' LIMIT {$offset},{$limit}";
                ?>
                <h2 class="mt-1">Search term : <?php echo $search_term; ?></h2>
                <hr>
                <?php
                ...
                <div class="d-flex pb-4 justify-content-center">
                    <div class="w-25 btn-group " role="group">
                        <?php
                        $sql1 = "SELECT * FROM post LEFT JOIN user ON post.author=user.user_id LEFT JOIN category ON post.category=category.category_id WHERE post.author LIKE '%{$search_term}%' OR post.title LIKE '%{$search_term}%' OR user.username LIKE '%{$search_term}%' OR category.category_name LIKE '%{$search_term}%'";
                        ...
                        if (mysqli_num_rows($result1) > 0) {
                            ...
                            if ($page > 1) {
                                echo  '<button type="button" class=" btn btn-primary"><a class="text-white text-decoration-none" href="search.php?search=' . $search_term . '&page=' . ($page - 1) . '">Prev</a></button>';
                            }
                            for ($i = 1; $i <= $total_page; $i++) {
                                ...
                                echo ' <button type="button" class="btn btn-primary ' . $active . '"><a class="text-white text-decoration-none" href="search.php?search=' . $search_term . '&page=' . $i . '">' . $i . '</a></button>';
                            }
                            if ($total_page > $page) {
                                echo  '<button type="button" class="btn btn-primary"><a class="text-white text-decoration-none" href="search.php?search=' . $search_term . '&page=' . ($page + 1) . '">Next</a></button>';
                            }
                        } else {
                            echo "";
                        } ?>
